...
public:
	...
	int cant_discos;
	disco() {}
	disco(int discos, int platos, int sectores) : cant_discos(discos),cant_sect(platos*sectores) {}
	long long capacidad(int tam) {return cant_sect*tam;}
};
...
{
	std::cout << "\nFASE 2: DISCO\n";
	int cantdiscos, cantplatos, cantpistas, cantsectores, tamsector;
	...
	disco_virtual *MIDISCO = new disco_virtual[cantdiscos];
	for (int i = 0; i < cantdiscos; i++)
	{
		MIDISCO[i].SC = sector(cantsectores,tamsector);
		MIDISCO[i].PS = pista(cantpistas,cantsectores);
		MIDISCO[i].PL = plato(cantplatos,cantpistas,MIDISCO[i].PS.cant_sect);
		MIDISCO[i].DS = disco(cantdiscos,cantplatos,MIDISCO[i].PL.cant_sect);
	}
...
void cargar_datos(int& byte_registro, int& cant_registros, dato* &columnas, int& cant_columnas)
{
	...
	while (esq >> noskipws >> letra)
	{
		if (letra == ' ')
		{
			...
			while (letra != '(')
			{
				tipo = (char*)realloc(tipo, sizeof(char)*(++cant_letras));
				tipo[cant_letras-2] = letra;
				esq >> noskipws >> letra;
			}
			...
			*/
			...
			columnas = (dato*)realloc(columnas, sizeof(dato)*(++cant_columnas));
...
void cargar_sectores(disco_virtual *MIDISCO, const int& cant_registros, const int& byte_registro)
{
	...
	int sec_cargados = 0;
	int registros_sector = MIDISCO[0].SC.capacidad/byte_registro;

	std::cout << "Cargando sectores...\n";
	std::cout << "Registros por sector: " << registros_sector << endl;
	char sec_carga[byte_registro*8*registros_sector];
	...
	while(sec_cargados*registros_sector < cant_registros)
	{
		...
		for (int i = 0; i < registros_sector; i++)
		{
			if (base.peek() == EOF) break;
			base.read(sec_carga,byte_registro*8);
			file_sect.write(sec_carga,byte_registro*8);
		}
...
void imprimir_sector(disco_virtual *MIDISCO,const int byte_registro, dato* &columnas, const int& cant_columnas)
{
	...
	{
		std::cin >> s;
		if (s > MIDISCO[0].DS.cant_sect || s <= 0)
			std::cout << "INGRESE UN VALOR VALIDO!\n";
		else break;
	}
	...
	int d = s/MIDISCO[0].DS.cant_sect;
	if (s%MIDISCO[0].DS.cant_sect != 0) d++;
	std::cout << "DISCO: " << d << endl;
	if (s%MIDISCO[0].DS.cant_sect != 0) s = s%MIDISCO[0].DS.cant_sect;

	int p = s/MIDISCO[0].PL.cant_sect;
	if (s%MIDISCO[0].PL.cant_sect != 0) p++;
	std::cout << "PLATO: " << p << endl;
	if (s%MIDISCO[0].PL.cant_sect != 0) s = s%MIDISCO[0].PL.cant_sect;

	std::cout << "SUPERFICIE: ";
	if (MIDISCO[0].PL.cant_sect/2 < s)
	{
		std::cout << "INFERIOR";
		s -= MIDISCO[0].PL.cant_sect/2;
	}
	else std::cout << "SUPERIOR";
	std::cout << endl;
	
	int ps = s/MIDISCO[0].PS.cant_sect;
	if (s%MIDISCO[0].PS.cant_sect != 0) ps++;
	std::cout << "PISTA: " << ps << endl;
	if (s%MIDISCO[0].PS.cant_sect != 0) s = s%MIDISCO[0].PS.cant_sect;
...
void cargar_bloques(disco_virtual *MIDISCO, const int& byte_registro)
{
	...
	std::cin >> sxb;
	MIDISCO[0].BL = bloque(sxb,MIDISCO[0].DS.cant_sect);
	std::cout << "Capacidad del bloque: " << MIDISCO[0].BL.capacidad(MIDISCO[0].SC.capacidad) << endl;
	std::cout << "Registros por bloque: " << MIDISCO[0].BL.cant_sect*(MIDISCO[0].SC.capacidad/byte_registro) << endl;
	std::cout << "Cantidad de bloques: " << MIDISCO[0].BL.cant_bloques << endl;
}
...
void imprimir_bloque(disco_virtual *MIDISCO, const int& byte_registro, dato* &columnas, const int& cant_columnas)
{
	...
	{
		std::cin >> b;
		if (b > MIDISCO[0].BL.cant_bloques || b <= 0)
			std::cout << "INGRESE UN VALOR VALIDO!\n";
		else break;
	}

	int s = (b-1)*MIDISCO[0].BL.cant_sect+1;
	...
	if ((s+MIDISCO[0].BL.cant_sect)-1 > MIDISCO[0].DS.cant_sect) std::cout << MIDISCO[0].DS.cant_sect;
	else std::cout << (s+MIDISCO[0].BL.cant_sect)-1;
	std::cout << endl;
	
	for (int i = 0; i < MIDISCO[0].BL.cant_sect; i++)
	{
